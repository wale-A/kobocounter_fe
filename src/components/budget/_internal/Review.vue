  ...
  <div class="review">
    ...
    <div class="review__detail">
      <Field
        ...
        :value="getFormattedAmount(total)"
        orientation="row"
      />
      <Field
        label="Start Date"
  ...
  computed: {
    total() {
      ...
      return this.budget.items
        .reduce((acc: number, category: categoryType) => {
          return (acc += parseFloat(category.value?.replace(/,/g, "")));
        }, 0)
        .toLocaleString();
    },
  },
...
export default class Review extends Vue {
  ...
  getFormattedAmount(amt: string | null): string {
    const amount = Math.abs(parseFloat((amt || "0")?.replace(/,/g, "")) || 0);
